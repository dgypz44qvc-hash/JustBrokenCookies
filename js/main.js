...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ---- MOBILE ROSE BRAND MOTIF SYSTEM ----
  // Uses exact image asset: images/rose-jbc-brown-background.png
  // Mobile-only visual insertions. Does not create new sections.
  (function addMobileRoseMotifs() {
    const roseSrc = 'images/rose-jbc-brown-background.png';
    function createRose(className) {
      const img = document.createElement('img');
      img.src = roseSrc;
      img.alt = '';
      img.className = `rose-mark ${className}`;
      img.setAttribute('aria-hidden', 'true');
      return img;
    }
    function createRoseDivider(className = '') {
      const divider = document.createElement('div');
      divider.className = `rose-divider ${className}`;
      divider.innerHTML = `
        <span class="rose-divider-line"></span>
        <img src="${roseSrc}" alt="" class="rose-mark rose-divider-icon" aria-hidden="true">
        <span class="rose-divider-line"></span>
      `;
      return divider;
    }
    // 1. Hero / intro rose seal.
    // Try to place it before the main CTA button.
    const possibleHeroButtons = Array.from(document.querySelectorAll('a, button'))
      .filter(el => {
        const text = el.textContent.trim().toLowerCase();
        return text.includes('start a project') || text.includes("let's talk") || text.includes('view all work');
      });
    const startProjectBtn = possibleHeroButtons.find(el =>
      el.textContent.trim().toLowerCase().includes('start a project')
    ) || possibleHeroButtons[0];
    if (startProjectBtn && !document.querySelector('.rose-hero-seal')) {
      const heroSealWrap = document.createElement('div');
      heroSealWrap.className = 'rose-hero-seal-wrap';
      heroSealWrap.appendChild(createRose('rose-hero-seal'));
      startProjectBtn.parentNode.insertBefore(heroSealWrap, startProjectBtn);
    }
    // 2. Selected Work divider.
    const selectedWorkHeading = Array.from(document.querySelectorAll('h1, h2, h3, .section-title, [class*="title"]'))
      .find(el => el.textContent.trim().toLowerCase().includes('selected work'));
    if (selectedWorkHeading && !document.querySelector('.rose-selected-divider')) {
      selectedWorkHeading.insertAdjacentElement(
        'afterend',
        createRoseDivider('rose-selected-divider')
      );
    }
    // 3. Testimonial watermark.
    const testimonialSection = Array.from(document.querySelectorAll('section, div, article'))
      .find(el => {
        const text = el.textContent.toLowerCase();
        return text.includes('testimonial') || text.includes("they didn't just tell our story");
      });
    if (testimonialSection && !testimonialSection.querySelector('.rose-testimonial-watermark')) {
      testimonialSection.classList.add('rose-section-anchor');
      testimonialSection.appendChild(createRose('rose-testimonial-watermark'));
    }
    // 4. Process cards, small rose top-right of Define / Design / Describe cards.
    const processCards = Array.from(document.querySelectorAll('article, .card, .process-card, .service-card, .value-card, [class*="card"]'))
      .filter(card => {
        const text = card.textContent.toLowerCase();
        return text.includes('define') || text.includes('design') || text.includes('describe');
      });
    processCards.forEach((card, index) => {
      if (!card.querySelector('.rose-card-mark')) {
        card.classList.add('rose-card-anchor');
        const rose = createRose('rose-card-mark');
        rose.classList.add(`rose-card-mark-${index + 1}`);
        card.appendChild(rose);
      }
    });
    // 5. How We Build Stories / process background watermark.
    const processSection = Array.from(document.querySelectorAll('section, .section, .process, [class*="process"]'))
      .find(section => {
        const text = section.textContent.toLowerCase();
        const hasBuildStories = text.includes('how we build stories');
        const hasAllSteps = text.includes('define') && text.includes('design') && text.includes('describe');
        return hasBuildStories || hasAllSteps;
      });
    if (processSection && !processSection.querySelector('.rose-process-watermark')) {
      processSection.classList.add('rose-section-anchor');
      processSection.appendChild(createRose('rose-process-watermark'));
    }
    // 6. Footer seal.
    const footer = document.querySelector('footer, .footer, [class*="footer"]');
    if (footer && !footer.querySelector('.rose-footer-seal')) {
      footer.classList.add('rose-section-anchor');
      const footerSeal = createRose('rose-footer-seal');
      footer.insertBefore(footerSeal, footer.firstChild);
    }
  })();
