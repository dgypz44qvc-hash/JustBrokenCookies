...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (revealElements.length > 0) {
    revealElements.forEach(el => {
      ...
      /* Skip gallery track cards — they're inside a sticky container and are
         revealed by the horizontal scroll, not by vertical intersection */
      if (el.closest('#featured-work .jbc-gallery-track')) return;
...
(function initHorizontalGallery() {
  ...
  /* ---- Measure & set section height for scroll travel ---- */
  function setup() {
    track.style.transform = '';
    lerpX = 0;
    travel = Math.max(0, track.scrollWidth - window.innerWidth);
    if (travel > 0) {
      /* Section needs to be tall enough: 100vh (for sticky window) + travel distance */
      section.style.minHeight = (window.innerHeight + travel) + 'px';
    } else {
      section.style.minHeight = '';
    }
  }

  /* ---- Convert horizontal drag/wheel into equivalent page scroll ---- */
  /* This keeps everything in sync — page scroll IS the source of truth. */
  var dragging      = false;
  var dragStartX    = 0;
  var dragScrollY   = 0;
  ...
  window.addEventListener('mousemove', function(e) {
    ...
    window.scrollTo({ top: dragScrollY + dx, behavior: 'instant' });
  });
  ...
  /* Horizontal trackpad / mouse wheel swipe → scroll the page vertically */
  pin.addEventListener('wheel', function(e) {
    var absX = Math.abs(e.deltaX);
    var absY = Math.abs(e.deltaY);
    /* Only handle clear horizontal intent */
    if (absX < 2 && absX <= absY) return;
    e.preventDefault();
    window.scrollBy({ top: e.deltaX, behavior: 'instant' });
  }, { passive: false });

  /* ---- Per-image parallax: images shift ±15% relative to card position ---- */
  /* Creates the "one continuous banner" effect as cards slide past each other. */
  function applyParallax() {
    var vw     = window.innerWidth;
    var center = vw * 0.5;
    track.querySelectorAll('.portfolio-item').forEach(function(item) {
      var rect  = item.getBoundingClientRect();
      /* t: -1 (card fully left) → 0 (card centred) → +1 (card fully right) */
      var t     = clamp(-1, 1, (rect.left + rect.width * 0.5 - center) / (vw * 0.5));
      var shift = -t * 15;            /* ±15% within the 30% overflow room */
  ...
  function render() {
    if (travel > 0) {
      /* Compute how far we are into the section's scroll travel */
      var sectionTop = section.getBoundingClientRect().top;
      /* sectionTop starts at positive (section below fold), goes negative as we scroll past */
      var rawTarget  = clamp(0, travel, -sectionTop);
      lerpX = lerp(lerpX, rawTarget, 0.09);
      if (Math.abs(lerpX - rawTarget) < 0.3) lerpX = rawTarget;

      track.style.transform = 'translateX(' + (-lerpX) + 'px)';
      if (progress) progress.style.width = (lerpX / travel * 100) + '%';
      applyParallax();
    }
