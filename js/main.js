...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (function initSelectedWorkCinema() {
    ...
    const updateCinema = () => {
      ...
      const creationsReveal = clamp((window.innerHeight - rect.top) / (window.innerHeight * 0.72), 0, 1);
      root.style.setProperty('--creations-title-reveal', creationsReveal.toFixed(4));
...
/* =========================================================
   Hero smooth scroll-away slide
   ========================================================= */
(function initHeroScrollSlide() {
  const hero = document.querySelector('.hero');
  if (!hero) return;
  ...
  const heroImage = hero.querySelector('.jbc-custom');
  const heroTag = hero.querySelector('.tag');
  const heroBottom = hero.querySelector('.hero-bottom');
  ...
  function updateHeroSlide() {
    ...
    hero.style.transform = `translate3d(0, ${eased * -42}px, 0)`;
    hero.style.opacity = `${1 - eased * 0.18}`;
    hero.style.filter = `blur(${eased * 1.2}px)`;

    if (heroMega) {
      heroMega.style.transform = `translate3d(0, ${eased * -90}px, 0) scale(${1 - eased * 0.035})`;
      heroMega.style.opacity = `${1 - eased * 0.24}`;
    }

    if (heroImage) {
      heroImage.style.transform = `translate3d(0, ${eased * -55}px, 0) scale(${1 + eased * 0.025})`;
      heroImage.style.opacity = `${1 - eased * 0.08}`;
    }

    if (heroTag) {
      heroTag.style.transform = `translate3d(0, ${eased * -36}px, 0)`;
      heroTag.style.opacity = `${1 - eased * 0.4}`;
    }

    if (heroBottom) {
      heroBottom.style.transform = `translate3d(0, ${eased * -28}px, 0)`;
      heroBottom.style.opacity = `${1 - eased * 0.48}`;
    }
  ...
  window.addEventListener('scroll', onScroll, { passive: true });
  window.addEventListener('resize', updateHeroSlide, { passive: true });
...
/* =========================================================
   JBC GLIMPSES IMAGE PATH FIX, SAFE
   Uses the 10 real images from /images.
   Does NOT touch carousel transforms, drag logic, rotation, or movement.
   ========================================================= */
(function () {
  const JBC_GLIMPSES_IMAGES = [
  ...
  function fixGlimpsesImagesOnly() {
    const cardsWrap = document.querySelector('#jbc-gs-cards');
    if (!cardsWrap) return;

    let cards = Array.from(cardsWrap.querySelectorAll('.jbc-gs-card'));

    if (cards.length < JBC_GLIMPSES_IMAGES.length) {
      const firstCard = cards[0];

      if (!firstCard) return;

      while (cards.length < JBC_GLIMPSES_IMAGES.length) {
        const clone = firstCard.cloneNode(true);
        cardsWrap.appendChild(clone);
        cards = Array.from(cardsWrap.querySelectorAll('.jbc-gs-card'));
      }
    }

    cards.slice(0, JBC_GLIMPSES_IMAGES.length).forEach((card, index) => {
      ...
      img.src = JBC_GLIMPSES_IMAGES[index];
      img.alt = '';
    ...
    console.log('JBC Glimpses image paths fixed:', JBC_GLIMPSES_IMAGES.length);
  }

  document.addEventListener('DOMContentLoaded', fixGlimpsesImagesOnly);
  window.addEventListener('load', fixGlimpsesImagesOnly);
  setTimeout(fixGlimpsesImagesOnly, 700);
})();
