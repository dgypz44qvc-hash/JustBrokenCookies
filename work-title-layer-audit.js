...
(async () => {
  ...
  const report = await page.evaluate(() => {
    ...
    const inspected = selectors.map(getInfo);

    const title = document.querySelector('#featured-work .jbc-work-created-title h2, #featured-work .sec-header h2');
    let stackAtTitle = [];
    ...
    let stackNearTopOfWork = [];

    if (featured) {
      const r = featured.getBoundingClientRect();
      const x = Math.round(r.left + 180);
      const y = Math.round(r.top + 80);

      stackNearTopOfWork = document.elementsFromPoint(x, y).slice(0, 18).map(el => ({
        tag: el.tagName.toLowerCase(),
        id: el.id || '',
        className: typeof el.className === 'string' ? el.className : '',
        text: (el.innerText || el.textContent || '').trim().slice(0, 120),
  ...
  await page.screenshot({
    path: 'work-title-layer-audit.png',
    fullPage: true
  });

  await browser.close();
  console.log('\nSaved screenshot: work-title-layer-audit.png');
})();
